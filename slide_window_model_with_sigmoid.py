...
# Required parameters
flags.DEFINE_string(
    "data_dir", './slide_window_model_with_sigmoid',
...
flags.DEFINE_string(
    "output_dir", './slide_window_model_with_sigmoid',
...
class i2b2_2012_Processor:
    ...
    def get_train_examples(self, data_dir):
        ...
        texts, labels, tar_event_labels = \
            i2b2_2012_data_preprocess.generate_slide_window_dataset('./i2b2_2012/train_data',
                                                                    self.get_be_labels(),
                                                                    self.get_sw_labels(),
    ...
    def get_dev_examples(self, data_dir):
        examples = []
        texts, labels, tar_event_labels = \
            i2b2_2012_data_preprocess.generate_slide_window_dataset('./i2b2_2012/test_data',
                                                                    self.get_be_labels(),
                                                                    self.get_sw_labels(),
                                                                    FLAGS.max_window_length,
                                                                    FLAGS.max_seq_length,
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.name_scope('vector_response'), tf.variable_scope('vector_response'):
        ...
        output_layer = tf.layers.dense(net, hidden_size)
        # output_layer = net

        for i in range(1, FLAGS.max_window_length):
            net = tf.layers.conv1d(inputs=net, filters=hidden_size, kernel_size=2, strides=1, padding='valid',
                                   kernel_initializer=initializers.xavier_initializer(), name='conv_layer_' + str(i))
    ...
    with tf.name_scope('loss'), tf.variable_scope("loss"):
        ...
        # [batch_size, seq_length, num_label]
        per_loss = tf.losses.sigmoid_cross_entropy(logits=logits, multi_class_labels=labels, reduction='none')
        ...
        total_loss = 4 * tf.reduce_mean(per_loss)

    pred_prob = tf.sigmoid(logits, 'pred_prob')
...
def main(_):
    ...
    if FLAGS.do_train and FLAGS.do_eval:
        early_stopping = tf.contrib.estimator.stop_if_no_decrease_hook(
            ...
            max_steps_without_decrease=8000,
            min_steps=1000
        )
    ...
    if FLAGS.do_predict:
        ...
        slide_window_model_evaluate.evaluate_with_slide_window(result, label_list, tokenizer, task_name=FLAGS.task_name)
